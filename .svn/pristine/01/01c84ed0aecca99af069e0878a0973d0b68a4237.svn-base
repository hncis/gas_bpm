...
function setChangeImg(){
	...
	for(var i=0;i<ids.length;i++){
		if(getColValue("doc_no", i+1, gridName1)!=""){
			var imgSrc = "";			
			imgSrc = "<img src='../../images/hncis_bttn/download_n.gif' onClick='doFileDown(\""+getColValue("ogc_fil_nm", i+1, gridName1)+"\"," + getColValue("seq", i+1, gridName1) + ");'/>";	
...
function doFileDown(file_name, seq){
		...
		$("#pop_yes").click(function(){
			$.unblockUI({
				onUnblock: function(){
					...
					doCommonAjax("doDeleteTxToFile.do", paramData, "deleteCallBack(jsonData.sendResult);");
				}
			});
		});
	}
...
function doSearch(){
	...
	doCommonSearch("doSearchTxToFile.do", util.jsonToString(params), "searchCallBack();", gridName1);
}