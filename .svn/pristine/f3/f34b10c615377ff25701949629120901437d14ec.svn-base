...
function init(){
	...
	cm = [
	        ...
	        {name:'meal_req_cnt',index:'meal_req_cnt', formatter:'string',width:120,align:'center',editable:false,sortable:false},
			{name:'vsit_purp_cd',index:'vsit_purp_cd',edittype:'select',formatter: "select",editable:false,width:180,align:'center',sortable:false,
				editoptions:{value:getComboValue('cb_vsit_purp_cd'),dataInit: function(elem) {$(elem).width(180);}}
			}, 
...
function doSearch(msgFlag){
	...
	doCommonSearch("doSearchGridEmToListForMeal.do", util.jsonToString(params), "loadCallBack();", "htmlTable", msgFlag);
}
...
function deptFind(){
	var param = "?dcd=key_ops_cd&dcdNm=key_ops_nm";
