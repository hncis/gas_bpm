...
function init(){
	...
	gridParam = {
		viewEdit : [{
			gridName     : gridName1,
			url          : "doSearchMaintenanceToFile.do",
			colNames     : cn,
			colModel     : cm,
			width        : "400",
			height       : "100%",
	...
	$("#pop_yes").click(function(){
		$.unblockUI({
			onUnblock: function(){
				...
				frm.action = "doSaveMaintenanceToFile.do";
				frm.submit();
			}
		});
	});
...
function doFileDown(file_name){
	...
	var frm = document.mainForm;
	frm.fileInfo.value = util.jsonToString(fileInfo);
	frm.action = "doMaintenanceFileDown.do";
	frm.submit();
		...
		$("#pop_yes").click(function(){
			$.unblockUI({
				onUnblock: function(){
					...
					doCommonAjax("doDeleteMaintenanceToFile.do", paramData, "deleteCallBack(jsonData.sendResult);");
				}
			});
		});
	}
...
function doSearch(){
	...
	doCommonSearch("doSearchMaintenanceToFile.do", util.jsonToString(params), "searchCallBack();", gridName1);
}