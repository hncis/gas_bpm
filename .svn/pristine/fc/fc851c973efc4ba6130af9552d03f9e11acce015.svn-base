...
function fnSetDocumentReady(){
	...
	//공통 콤보 조회
	getCommonCode("keyCategory:DB01;", "N", "searchDeptComb();");
}

function searchDeptComb(){
	var params = {
		};
	doCommonAjax("doSearchDbToDeptCombo.do", params,"deptComboCallBack(jsonData);");
}
...
function init(){
	...
	plot1 = $.jqplot('chart1', [mon_arr], {
		title: $("#keyCategory option:selected").text(),
		seriesDefaults:{
...
function doSearch1(){
	var keyData = {
			...
			key_category	: $('#keyCategory').val(),
			key_dept_cd		: $('#keyDeptCd').val()
	};
	paramData = {
			paramJson      	: util.jsonToString(keyData)
	};
	doCommonAjax("doSearchDbToDashBoard.do", paramData,"loadCallBack(jsonData.sendResult);");
}
/**
 * callback
 */
