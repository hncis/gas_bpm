...
function fnSetDocumentReady(){
	...
	getCommonCode("key_produto::Y:PD;", "N", "getGridCommonCode();", "/hncis/officeSupplies/getCatgCombo.do");
}
...
function init(){
	...
	$("#" + gridName).setColProp('eeno', {editoptions:{maxlength:"8", dataEvents:[{type:"keyup",
    	fn:function(e){
    		var row = $(e.target).closest("tr.jqgrow");
    		var rowId = row.attr("id");
...
function doSearch(){
	doCommonSearch("doSearchByXos06.do", util.jsonToString(fnParamJson()), "loadCallBack();", gridName, "N");
}
		...
		$("#pop_yes").click(function(){
			$.unblockUI({
				onUnblock: function(){
					...
					doCommonAjax("doInsertByXos06.do", paramData, "setBottomMsg(jsonData.sendResult.message, true);doSearch();");
				}
			});
		});
	}
		...
		$("#pop_yes").click(function(){
			$.unblockUI({
				onUnblock: function(){
					...
					doCommonAjax("doDeleteByXos06.do", paramData, "setBottomMsg(jsonData.sendResult.message, true);doSearch();");
				}
			});
		});
	}
