...
<script>
	...
	$(document).ready(function() {
		...
		fn_getCompletedInstaceList("${command.pageNo}");
	...
	var fn_getMyCompletedInstanceListCount = function() {
		$.ajax({
			data : JSON.stringify($("#serachForm").serializeArray()),
			type : "POST",
			url : "<c:url value='/delegated/instancelist/count/kriss/completed/instance' />",
			cache : false,
			dataType : "JSON",
	...
	var fn_getCompletedInstaceList = function(page) {
		if (!page ) page = 1;
		$("#pagerFrame").attr("src","");
		$.ajax({
			data : JSON.stringify($("#serachForm").serializeArray()),
			type : "POST",
			url : "<c:url value='/delegated/instancelist/list/kriss/completed/instance/' />"+page+"/"+pageSize,
			cache : false,
			dataType : "JSON",
			contentType : "application/json;charset=utf-8;",
			beforeSend: function(){
			...
			success : function(data) {
				pageNo = page;
				fn_getMyCompletedInstanceListCount();
			    ...
			    for ( var i = 0; i < data.length; i++ ){
			    	...
			    	//strFinishedDate
			    	var strFinishedDateChild = $("<td></td>").attr("style","vertical-align: middle; padding:2px;");
			    	var strFinishedDateFont = $("<font></font>").attr("size","2").html(data[i].strFinishedDate);
			    	strFinishedDateFont.addClass("list_font");
			    	strFinishedDateChild.append(strFinishedDateFont);
			    	tr.append(strFinishedDateChild);
			    	//status
			    	var statusChild = $("<td></td>").attr("style","vertical-align: middle; padding:2px;");
			    	var statusFont = $("<font></font>").attr("size","2").html(data[i].status);
...
<body style="font-size: 12px;">
	<form:form name="workListForm" id="workListForm" method="post" target="_blank" command="command">
	...
	<div class="container-fluid">
		<table width=100%>
			...
			<tr>
				<td>
				<form:form name="serachForm" id="serachForm" method="post" target="_blank" command="command">
					<table class="table">
						<tr style="border-bottom: 1px solid #ddd;">
							...
							<td class="active" rowspan="2" style="vertical-align: middle; text-align:center;">
								<button type="button" class="btn btn-primary" onClick="javascript:fn_getCompletedInstaceList();">검색</button>
							</td>
						</tr>
						<tr style="border-bottom: 1px solid #ddd;">
							<td class="active" style="vertical-align: middle"><spring:message code="old.instancelist.instance.eddt.label" 	/></td>
							<td style="vertical-align: middle" colspan="3">
								<form:input type="text" id="strFromDate" path="strFromDate" name="strFromDate" style="width:80px" readonly="false" />
								~
								<form:input type="text" id="strToDate" name="strToDate" path="strToDate" style="width:80px" readonly="false" />
							</td>
						</tr>
					</table>
				</form:form>
					...
					<table class="table table-striped table-inverse">
					  <thead>
					    <tr>
					      ...
					      <th style="background-color: #454545; height: 20px"><font size="2" color="#ffffff"><spring:message code="old.instancelist.instance.eddt.label" 	/>				</font></th>
					      <th style="background-color: #454545; height: 20px"><font size="2" color="#ffffff"><spring:message code="old.instancelist.instance.completed.status.label" 	/>				</font></th>
			...
			<tr><td>
				<form:form name="pagerForm" id="pagerForm" command="command" target="pagerFrame" action="" method="POST">
					...
					<form:input type="hidden" path="callbackFunc" name="callbackFunc" id="callbackFunc" value="fn_getCompletedInstaceList" />
				</form:form>
