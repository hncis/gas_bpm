...
function init(){
	...
	var cn        = ["DocNo.", "Eeno", "Seq", "카테고리", "종류", "시작일자", "종료일자", "결제종류", "통화", "환율", "금액", "National Payment", "Sap", "영수증", "비고"];
	var cm =
		[
			...
			{name:'prvs_scn_cd',	index:'prvs_scn_cd', sortable:false,		formatter:"select",	width:200,	align:'center',	editable:true,	frozen:true,	edittype:'select', 
...
//		                    multiComboController(gridName1, $(e.target).closest('tr.jqgrow'), 0, "prvs_scn_cd:prvs_dtl_cd", "comboVal1", "prvs_dtl_cd");
			...
			{name:'prvs_dtl_cd',	index:'prvs_dtl_cd', sortable:false,		formatter: "select",	width:200,		align:'center',	editable:true,	frozen:true,
				edittype:'select', 
...
//		                    multiComboController(gridName1, $(e.target).closest('tr.jqgrow'), 0, "prvs_scn_cd:prvs_dtl_cd", "comboVal1", "prvs_dtl_cd");
			...
			{name:"strt_ymd",	index:"strt_ymd"	, sortable:false,	formatter:"string",		width:70,	align:"center",	editable:true,	frozen : false,
				editoptions: {
					readonly : true,
			...
			{name:"fnh_ymd",	index:"fnh_ymd"	, sortable:false,	formatter:"string",		width:70,	align:"center",	editable:true,	frozen : false,
				editoptions: {
					readonly : true,
		            dataInit: function(element) {
		     		    $(element).datepicker({
			...
			{name:'stl_way_cd',	index:'stl_way_cd', sortable:false,		formatter: "select",	width:90,	align:'center',	editable:true,	edittype:'select', 
			...
			{name:"rem_sbc",	index:"rem_sbc", sortable:false,	formatter:"string",		width:190,	align:"left",	editable:true,	frozen : false}
		];
			...
			if(rowId){
				if(!(Number(sess_auth) > 4 || sess_mstu == "M")){
					if(getColValue("arsl_refl_yn", rowId) == "Y"){
						alertUI(rowId + " 행 : 데이터를 저장 할 수 없습니다.");
						return;
					}
				}
				if(getColValue("prvs_dtl_cd", rowId) == ""){
					alertUI(rowId + " 행 : 종류를 선탣해 주세요.");
					return;
				}
				if(getColValue("strt_ymd", rowId) == ""){
					alertUI(rowId + " 행 : 시작일자를 입력해 주세요.");
					return;
				}
				if(getColValue("fnh_ymd", rowId) == ""){
					alertUI(rowId + " 행 : 종료일자를 입력해 주세요.");
					return;
				}
				if(getColValue("stl_way_cd", rowId) == ""){
					alertUI(rowId + " 행 : 결제종류를 선택해 주세요.");
					return;
				}
				if(getColValue("curr_cd", rowId) == ""){
					alertUI(rowId + " 행 : 통화를 선택해 주세요.");
					return;
				}
				if(getColValue("apl_xr", rowId) == ""){
					alertUI(rowId + " 행 :  환율을 입력해 주세요.");
					return;
				}
				if(getColValue("prvs_amt", rowId) == ""){
					alertUI(rowId + " Pedagio :  금액을 입력해 주세요.");
					return;
				}
...
//	if($("#hid_pgs_st_cd").val() == "3"){
//		$("#btn_s").attr('style', 'display:none;');
//		$("#btn_d").attr('style', 'display:none;');
