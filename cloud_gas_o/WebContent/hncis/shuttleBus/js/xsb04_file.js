...
function init(){
	...
	gridParam = {
		viewEdit : [{
			gridName     : gridName1,
			url          : "doSearchShuttleBusToFile.do",
			colNames     : cn,
			colModel     : cm,
			width        : "400",
			height       : "100%",
	...
	$("#pop_yes").click(function(){
		$.unblockUI({
			onUnblock: function(){
				...
				frm.action = "doSaveShuttleBusToFile.do";
				frm.submit();
			}
		});
	});
...
function doFileDown(file_name){
	...
	var frm = document.mainForm;
	frm.fileInfo.value = util.jsonToString(fileInfo);
	frm.action = "doShuttleBusFileDownPopup.do";
	frm.submit();
		...
		$("#pop_yes").click(function(){
			$.unblockUI({
				onUnblock: function(){
					...
					doCommonAjax("doDeleteShuttleBusToFile.do", paramData, "deleteCallBack(jsonData.sendResult);");
				}
			});
		});
	}
...
function doSearch(){
	...
	doCommonSearch("doSearchShuttleBusToFile.do", util.jsonToString(params), "searchCallBack();", gridName1);
}