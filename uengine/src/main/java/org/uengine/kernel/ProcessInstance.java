...
//	abstract public ProcessDefinition getProcessDefinition(String definitionId);
	...
	abstract public List<String> getMessageListeners(String message) throws Exception;
...
////////////////////////hot spots /////////////////////////////////
	...
	public List<HumanActivity> getAllRunningOrCompletedHumanActivities(ComplexActivity parent) throws Exception {
		List<HumanActivity> humActs = new ArrayList<HumanActivity>();
		Iterator<Activity> childs = parent.getChildActivities().iterator();
		while(childs.hasNext()) {
			Activity child = childs.next();
			if ( child instanceof HumanActivity && (child.getStatus(this).equals(Activity.STATUS_RUNNING) || child.getStatus(this).equals(Activity.STATUS_TIMEOUT) || child.getStatus(this).equals(Activity.STATUS_COMPLETED)) ) {
				humActs.add((HumanActivity) child);
			} else if ( child instanceof ComplexActivity ) {
				humActs.addAll(getAllRunningOrCompletedHumanActivities((ComplexActivity) child));
			}
		}
	...
	public List<HumanActivity> getAllHumanActivities(ComplexActivity parent) throws Exception {
		final List<HumanActivity> humActs = new ArrayList<HumanActivity>();
		Iterator<Activity> childs = parent.getChildActivities().iterator();
		while(childs.hasNext()) {
			Activity child = childs.next();
			if ( child instanceof HumanActivity ) {
				...
				humActs.addAll(getAllHumanActivities((ComplexActivity) child));
			}
		}
	...
	public List<Activity> getRunningOrCompletedActivities() throws Exception{
		final ProcessInstance finalThis = this;
		final List<Activity> findingActivities = new ArrayList<Activity>();
			...
			public void logic(Activity act){
				try{
					//if(!(act instanceof ComplexActivity)){
						...
						if(status.equals(Activity.STATUS_RUNNING) || status.equals(Activity.STATUS_TIMEOUT) || status.equals(Activity.STATUS_COMPLETED)){
	...
	public List<String> getRunningOrCompletedActivityTracingTags() throws Exception{
		List<Activity> activities = getRunningOrCompletedActivities();
		List<String> tracingTags = new ArrayList<String>();
		for(int i=0; i<activities.size(); i++){
			tracingTags.add(activities.get(i).getTracingTag());
		}
	...
	public EventHandler[] getEventHandlersInAction() throws Exception{
		...
		List<String> mls = getMessageListeners("event");
