...
//		}
	...
	public Vector<Activity> getChildActivities() {
...
*/		){
	...
	protected void gatherPropagatedActivitiesOf(ProcessInstance instance, Activity child, List<Activity> list) throws Exception{
		int where = childActivities.indexOf(child);
	...
	public ActivityReference getInitiatorHumanActivityReference(final ProcessInstance instance, final ProcessTransactionContext ptc) throws Exception{
...
//			throw new RuntimeException("this process definition is not allowed to be initiated by the first workitem.");
			List<ActivityReference> arList = new ArrayList<ActivityReference>();
			if ( instance == null && ptc == null ) {
				ProcessManagerBean bean = null;
				try {
					bean = new ProcessManagerBean();
					arList = getAllHumanActivities(null, bean.getTransactionContext());
				} catch (Exception e) {
					throw e;
				} finally {
					if ( bean != null)
			...
			} else if ( instance != null && ptc == null ) {
				arList = getAllHumanActivities(instance, instance.getProcessTransactionContext());
			} else {
				arList = getAllHumanActivities(null, ptc);
			}
			
			if ( arList.isEmpty() )
				return null;
			if ( instance == null && !arList.isEmpty() ){
				return arList.get(0);
			} else {
				ActivityReference result = null;
				//comleted, running first
				for(ActivityReference ar: arList){
					if ( result != null )
						break;
					Activity act = ar.getActivity();
					String instanceId = ar.getInstanceId();
					if ( instanceId == null ) {
						continue;
					} else {
						ProcessInstance actInstance = instance.getProcessTransactionContext().getProcessManager().getProcessInstance(instanceId);
						String actStatus = act.getStatus(actInstance);
						if (actStatus.equals(Activity.STATUS_COMPLETED) ||
								actStatus.equals(Activity.STATUS_RUNNING) ||
								actStatus.equals(Activity.STATUS_TIMEOUT))
							result = ar;
					}
				}
	...
	public List<ActivityReference> getAllHumanActivities(ProcessInstance instance, ProcessTransactionContext ptc) throws Exception {
		List<ActivityReference> humActs = new ArrayList<ActivityReference>();
		Iterator<Activity> childs = getChildActivities().iterator();
		while(childs.hasNext()) {
			Activity child = childs.next();
			if ( child instanceof HumanActivity ) {
				ActivityReference ar = new ActivityReference();
				ar.setActivity(child);
				ar.setAbsoluteTracingTag(child.getTracingTag());
				ar.setDefVerId(getProcessDefinition().getId());
				if ( instance != null)
					ar.setInstanceId(instance.getInstanceId());
				humActs.add(ar);
			} else if ( child instanceof SubProcessActivity ) {
				if ( instance != null) {
					List<ProcessInstance> subInsts = ((SubProcessActivity)child).getSubProcesses(instance);
					if (subInsts.isEmpty()){
						String defVerId = ((SubProcessActivity)child).getDefinitionIdOnly();
						ProcessDefinition subPd = ptc.getProcessManager().getProcessDefinition(defVerId);
						humActs.addAll(subPd.getAllHumanActivities(instance, ptc));
					} else {
						for(ProcessInstance subInstance: subInsts){
							humActs.addAll(subInstance.getProcessDefinition().getAllHumanActivities(subInstance, ptc));
						}
					}
				} else {
					String defVerId = ((SubProcessActivity)child).getDefinitionIdOnly();
			...
			} else if ( child instanceof ComplexActivity ) {
				humActs.addAll(((ComplexActivity) child).getAllHumanActivities(instance, ptc));
			}
		}
	...
	public List<Activity> getAllChildActivities(ProcessTransactionContext ptc) throws RemoteException {
		...
		for ( Iterator<Activity> i = getChildActivities().iterator(); i.hasNext(); ) {
			...
			childs.add(child);
			if ( child instanceof SubProcessActivity ) {
				String defVerId = ((SubProcessActivity)child).getDefinitionIdOnly();
				ProcessDefinition subPd = ptc.getProcessManager().getProcessDefinition(defVerId);
				childs.addAll(subPd.getAllChildActivities(ptc));
			} else if ( child instanceof ComplexActivity ) {
				childs.addAll(((ComplexActivity)child).getAllChildActivities(ptc));
			}
		}
		return childs;
	}

	public List<HumanActivity> getCompletedHumanActivities(ProcessInstance instance) throws Exception {
		List<HumanActivity> childs = new ArrayList<HumanActivity>();
		for ( Iterator<Activity> i = getChildActivities().iterator(); i.hasNext(); ) {
			Activity child = i.next();
			if (child instanceof HumanActivity && child.getStatus(instance).equals(Activity.STATUS_COMPLETED))
				childs.add((HumanActivity) child);
			else if ( child instanceof ComplexActivity && !(child instanceof SubProcessActivity) &&
					(child.getStatus(instance).equals(Activity.STATUS_COMPLETED) ||child.getStatus(instance).equals(Activity.STATUS_RUNNING))){
				childs.addAll(((ComplexActivity)child).getCompletedHumanActivities(instance));
			}
		}
		return childs;
	}

	public void attatchClonedChildActivity(Activity child) {
		child.setTracingTag(null);
		addChildActivity(child);
		if ( child instanceof ComplexActivity ) {
			List<Activity> clonedChildActivities = new ArrayList<Activity>();
			for (Activity clonedChild: ((ComplexActivity)child).getChildActivities()){
				clonedChildActivities.add((Activity) clonedChild.clone());
			}
			((ComplexActivity)child).getChildActivities().removeAllElements();
			for (Activity clonedChild: clonedChildActivities){
				((ComplexActivity)child).attatchClonedChildActivity(clonedChild);
			}
		}
