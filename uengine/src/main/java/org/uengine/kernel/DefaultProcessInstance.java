...
//System.out.println("EJBActivityInstance::getRoleMapping.class = " + val.getClass());
	...
	public void putRoleMapping(String name, RoleMapping roleMap) throws Exception{
		...
		//전체 휴먼을 찾아서 롤매핑의 첫번째 아이디를 업데이트함(진행중인것에 한함.
		Iterator<ActivityReference> i = getProcessDefinition().getAllHumanActivities(this, getProcessTransactionContext()).iterator();
		while (i.hasNext()) {
			HumanActivity humAct = (HumanActivity) i.next().getActivity();
			if ( humAct.getRole().getName().equals(name) && 
					(humAct.getStatus(this).equals(Activity.STATUS_RUNNING) || humAct.getStatus(this).equals(Activity.STATUS_TIMEOUT)) &&
					humAct.getTaskIds(this) != null &&
					humAct.getTaskIds(this).length > 0 ) {
				roleMap.beforeFirst();
				getWorkList().updateWorkItem(humAct.getTaskIds(this)[0], roleMap.getEndpoint(), null, getProcessTransactionContext());
			}
			humAct.firePropertyChangeEventToActivityFilters(this, "saveEndpoint", new ResultPayload());
		}
...
//////////////////
	...
	public DefaultProcessInstance(ProcessDefinition procDefinition, String instanceId, Map<String,Serializable> options) throws Exception{
	...
	public Map<String,Serializable> getAll(String scope) throws Exception{				
		return variables;
	}
	public Map<String,Serializable> getAll() throws Exception{				
		return getAll("");
	}
	...
	public List<String> getMessageListeners(String message) throws Exception{
		String keyStr = "MESSAGE_" + message;

		List<String> messageSubscriptions = null;

		String messageSubscriptionsString = (String)getProperty("", keyStr);
		if(messageSubscriptionsString!=null){
			messageSubscriptions = new ArrayList<String>();
		...
		if("event".equals(message) && (messageSubscriptions==null || messageSubscriptions.size()==0)){
			messageSubscriptions = new ArrayList<String>();
			final List<String> finalSubscribedScopes = messageSubscriptions;
...
/*		ProcessInstance inst = new DefaultProcessInstance();
	...
	public Object getBeanProperty(String key) {
		...
		try {
			...
			HashMap<String,Serializable> objects = new HashMap<String,Serializable>();
			objects.put("instance", this);
			objects.put("definition", getProcessDefinition());
			objects.put("activities", new ActivityBeanResolver(this));  
