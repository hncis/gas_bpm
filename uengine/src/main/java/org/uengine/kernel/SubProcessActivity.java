...
*/
  ...
  private Hashtable getUnInitiatedRoleMappings(ProcessInstance instance) throws Exception{
    ...
    List<ProcessInstance> subprocessInstanceList = getSubProcesses(instance);
    Hashtable boundRoleMappings = new Hashtable();
    for(int i=0; i < subprocessInstanceList.size();i++){
      ProcessInstance thePI = ((ProcessInstance)subprocessInstanceList.get(i));
      if(thePI.isRunning("")){
  ...
  private void stopSubProcessInstance(ProcessInstance instance,Hashtable deletedRoleMappings) throws Exception{
	    ...
	    List<String> spInstIds = getSubprocessIds(instance);
	    List<String> spInstLabels = getSubprocessIds(instance, SUBPROCESS_INST_LABELS);
	    List<ProcessInstance> subprocessInstanceList = getSubProcesses(instance);
	    for(int i=0; i < subprocessInstanceList.size();i++){
	      ProcessInstance thePI = ((ProcessInstance)subprocessInstanceList.get(i));
	      RoleMapping subRoleMapping = thePI.getRoleMapping(boundRoleName);
  ...
  public void refreshMultipleInstance(ProcessInstance instance)throws Exception{
   ...
   //------- add instance
    ...
    List<ProcessInstance> subprocessInstances = new ArrayList<ProcessInstance>();
    List<String> subprocessInstanceIds = new ArrayList<String>();
    List<String> subprocessLabels = new ArrayList<String>();
    ...
    for(int i=0; i<subprocessInstances.size(); i++){
      ProcessInstance thePI = ((ProcessInstance)subprocessInstances.get(i));
  ...
  public void executeActivity(ProcessInstance instance)
    ...
    List<ProcessInstance> subprocessInstances = new ArrayList<ProcessInstance>();
    List<String> subprocessInstanceIds = new ArrayList<String>();
    List subprocessLabels = new ArrayList();
...
//      if(roleMapping==null) throw new UEngineException("Mapping for the 'forEachRole'(" + getForEachRole().getDisplayName() + ") is not set yet. \n Please check if there exist bound user by query: select * from bpm_rolemapping where instanceid=" + instance.getInstanceId() + " and rolename='" + getForEachRole() + "' ;");
      if (roleMapping == null || roleMapping.size() == 0){
    	  fireComplete(instance);
    	  return;
      }
  ...
  protected void applyVariableBindings(ProcessInstance instance, List<String> spIds, Map subProcesses, Map options) throws Exception{
    ...
    for(int indexOfSP=0; indexOfSP<spIds.size(); indexOfSP++){
      String subProcessId = spIds.get(indexOfSP);
  ...
  protected void applyRoleBindings(ProcessInstance instance, List<String> spIds, Map subProcesses, Map options) throws Exception{
  ...
  public List<ProcessInstance> getSubProcesses(ProcessInstance instance) throws Exception{
    Hashtable options = new Hashtable();
    options.put("ptc", instance.getProcessTransactionContext());

    List<String> spIds = getSubprocessIds(instance, SUBPROCESS_INST_ID);
    List<ProcessInstance> subProcesses = new ArrayList<ProcessInstance>();
...
//						newRoleMapping.fill(instance.getProcessTransactionContext());
...
public Map getActivityDetails(ProcessInstance inst, String locale)
  ...
  public List<String> getSubprocessIds(ProcessInstance instance) throws Exception{
    return getSubprocessIds(instance, SUBPROCESS_INST_ID);
  }
  
  public List<String> getSubprocessLabels(ProcessInstance instance) throws Exception{
    return getSubprocessIds(instance, SUBPROCESS_INST_LABELS);
  }
  
  public List<String> getSubprocessIds(ProcessInstance instance, String space) throws Exception{
    ...
    List<String> listSpIds = new ArrayList<String>();
    if(spIds!=null)
    for(int i=0; i<spIds.length; i++){
    	listSpIds.add(spIds[i]);
    }
    
    return listSpIds;
  }

  protected void setSubprocessIds(ProcessInstance instance, List<String> spIds, String space) throws Exception{
		StringBuffer spId = new StringBuffer();
		for (int i = 0; i < spIds.size(); i++) {
			if (spId.length() > 0) spId.append(",");
			spId.append(spIds.get(i));
		}
  ...
  public void reset(ProcessInstance instance) throws Exception {
    if(!isRunAndForget()){
      List<ProcessInstance> subProcesses = getSubProcesses(instance);
  ...
  public void attachSubProcess(ProcessInstance instance, String subProcessInstanceId, String label) throws Exception{
    List<String> currSubProcessIds = getSubprocessIds(instance);
    List<String> currSubProcessLabels = getSubprocessLabels(instance);
  ...
  public void addMultipleInstanceByForEachVariable(ProcessInstance instance, Serializable theValue)throws Exception{
     ...
     List<String> subprocessInstanceIds = getSubprocessIds(instance,SUBPROCESS_INST_ID);
     List<String> subprocessLabels = getSubprocessIds(instance,SUBPROCESS_INST_LABELS);
     List<ProcessInstance> subprocessInstances = getSubProcesses(instance);
     
     ProcessInstance thePI = initiateSubProcess(null, instance, null, theValue, isConnectedMultipleSubProcesses, subprocessInstanceIds.size());
          
     subprocessInstances.add(thePI);
     subprocessInstanceIds.add(thePI.getInstanceId());
     subprocessLabels.add((String)theValue);
