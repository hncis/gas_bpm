...
public class ProcessDefinition extends ScopeActivity {
...
//		}
	...
	public ActivityReference getInitiatorHumanActivityReference(ProcessInstance instance) throws Exception{
		if(initiatorHumanActivity!=null){
		...
		ProcessTransactionContext ptc = null;
		if ( instance != null)
			ptc = instance.getProcessTransactionContext();
		ActivityReference activityReference = super.getInitiatorHumanActivityReference(instance, instance.getProcessTransactionContext());
...
//			if (!messageListenerVectors.containsKey(message)) return;
			List subscribedScopes = instance.getMessageListeners(message);
			
			if(subscribedScopes!=null)
			for(Iterator iter = subscribedScopes.iterator(); iter.hasNext(); ){
