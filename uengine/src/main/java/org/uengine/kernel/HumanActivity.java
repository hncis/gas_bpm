...
public class HumanActivity extends ReceiveActivity{
	...
	public static final String IS_AUTO_COMPLETED = "isAutoCompleted";
	...
	static{
		...
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue1", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue2", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue3", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue4", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue5", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue6", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue7", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue8", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue9", 0);
		org.uengine.kernel.descriptor.ActivityDescriptor.fieldOrder.insertElementAt("ExtValue10", 0);
	}
	...
	private boolean whenRoleNotBoundingSetLoggedUserRoleMapping = false;
		public boolean isWhenRoleNotBoundingSetLoggedUserRoleMapping() {
			return whenRoleNotBoundingSetLoggedUserRoleMapping;
		}
		public void setWhenRoleNotBoundingSetLoggedUserRoleMapping(
				boolean whenRoleNotBoundingSetLoggedUserRoleMapping) {
			this.whenRoleNotBoundingSetLoggedUserRoleMapping = whenRoleNotBoundingSetLoggedUserRoleMapping;
		}
...
//			System.out.println("dueDateInDate : " + dueDateInDate );
	...
	private void setCompletedRoleMapping(ProcessInstance instance, RoleMapping completedRoleMapping) throws Exception{
		RoleMapping completedRm = (RoleMapping)instance.getProcessTransactionContext().getProcessManager().getGenericContext().get(GENERICCONTEXT_CURR_LOGGED_ROLEMAPPING);
		instance.getWorkList().updateWorkItem(getTaskIds(instance)[0], completedRm.getEndpoint(), null, instance.getProcessTransactionContext());
...
//				throw new NullPointerException();
		...
		if(instance.isNew()){ //only when the instance is newly initiating now.
			ActivityReference actRef = getProcessDefinition().getInitiatorHumanActivityReference(instance);
	...
	protected void addWorkitem(ProcessInstance instance, String defaultStatus) throws Exception{
		...
		try{
			...
			if(roleMapping==null || roleMapping.getEndpoint()==null || roleMapping.getEndpoint().trim().length()==0){
				if (isWhenRoleNotBoundingSetLoggedUserRoleMapping()){
					RoleMapping newRm = (RoleMapping) instance.getProcessTransactionContext().getProcessManager().getGenericContext().get(GENERICCONTEXT_CURR_LOGGED_ROLEMAPPING);
					newRm.setName(getRole().getName());
					instance.putRoleMapping(getRole().getName(), newRm);
					roleMapping = newRm;
				} else
...
//		}while(roleMapping.next());
		...
		firePropertyChangeEventToActivityFilters(instance, "saveEndpoint", new ResultPayload());
...
//		saveSnapshotHTML(instance);
		...
		RoleMapping rm = (RoleMapping) instance.getProcessTransactionContext().getProcessManager().getGenericContext().get(GENERICCONTEXT_CURR_LOGGED_ROLEMAPPING);
		String userId = null;
		if ( rm != null && rm.size() > 0 ){
			rm.beforeFirst();
			userId = rm.getEndpoint();
		}
		wl.updateWorkItem(taskIds[0], userId, rp.getExtendedValues(), instance.getProcessTransactionContext());
	...
	public HumanActivity getPreviousHumanActivity(ProcessInstance instance) throws Exception{
		List<Activity> actList = getPreviousActivities();
