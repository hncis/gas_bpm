...
public class ProcessManagerBean implements SessionBean, SessionSynchronization, ProcessManagerRemote {
	...
	static boolean DEBUG = GlobalContext.getPropertyString("server.invocationlog", "false").equalsIgnoreCase("true");
	...
	private void completeInitiatorWorkitem(ProcessInstance instance, ResultPayload resultPayload) throws Exception {
		ProcessDefinition definition = instance.getProcessDefinition();
		ActivityReference initiatorHumanActivityReference = definition
				.getInitiatorHumanActivityReference(instance);
			...
			for (int i = 0; i < scopesByTracingTag.length - 1; i++) {
				...
				if (spInstanceIds.size() == 0) {
					throw new UEngineException(
							"Activity in the subprocess [" + absoluteTracingTag + "] cannot be found.");
				}
		...
		// (HumanActivity)initiatorHumanActivityReference.getActivity();
		...
		if (!(instance instanceof DefaultProcessInstance) && (taskIds == null || taskIds.length < 1))
			throw new UEngineException("The first human work didn't start properly.");

		if (resultPayload != null && taskIds != null)
			resultPayload.setExtendedValue(new KeyedParameter(HumanActivity.PAYLOADKEY_TASKID, taskIds[0]));
		//

		definition.fireMessage(message, instance, resultPayload);
	}

	public void executeEventByWorkitem(String mainInstanceId, String eventName, ResultPayload resultPayload)
			throws RemoteException {
		executeEventByWorkitem(mainInstanceId, eventName, null, resultPayload);
	}

	public void executeEventByWorkitem(String mainInstanceId, String eventName, String triggerActivityTracingTag,
			ResultPayload resultPayload) throws RemoteException {
		try {
			...
			if (handlerActivity instanceof SubProcessActivity) {
				...
				List<String> idVt = subProcessActivity.getSubprocessIds(mainProcessInstance);
			...
			if (handlerActivity instanceof HumanActivity) {
				...
				ActivityReference initiatorHumanActivityReference = complexActivity
						.getInitiatorHumanActivityReference(mainProcessInstance, mainProcessInstance.getProcessTransactionContext());
	...
	public void setProcessVariable(String instanceId, String scope, String varKey, Serializable val)
			throws RemoteException {
	...
	public Serializable getProcessVariable(String instanceId, String scope, String varKey) throws RemoteException {
		...
		try {
			...
			if (varKey.indexOf('.') < 0) {
				...
				if (variable == null)
					throw new UEngineException("Undeclared process variable reference : " + varKey);

				ProcessVariableValue theValue = definition.getProcessVariable(varKey).getMultiple(instance, scope);

				// ProcessVariableValue theValue = instance.getMultiple(scope,
				// varKey);
	...
	public RoleMapping getRoleMappingObject(String instanceId, String roleName, boolean isLog) throws RemoteException {
		...
		try {
			...
			roleMapping = definition.getRole(roleName).getMapping(instance);
			if (roleMapping == null)
				return null;
	...
	public ProcessDefinitionRemote[] listProcessDefinitionRemotesLight() throws RemoteException {
		try {
			ProcessDefinitionRepositoryHomeLocal pdhr = GlobalContext
					.createProcessDefinitionRepositoryHomeLocal(getTransactionContext());
			ProcessDefinitionVersionRepositoryHomeLocal pdihr = GlobalContext
					.createProcessDefinitionVersionRepositoryHomeLocal(getTransactionContext());
			...
			for (Iterator iter = definitions.iterator(); iter.hasNext();) {
				...
				if (!pdr.isFolder()) {
					...
					// UEngineUtil.getProcessNameAndVersion(pdr.getName());
					...
					for (Iterator iter2 = versions.iterator(); iter2.hasNext();) {
						ProcessDefinitionVersionRepositoryLocal pdirl = ((ProcessDefinitionVersionRepositoryLocal) iter2
								.next());
		...
		try {
			...
			if (productionVersionId == null || ((Long) productionVersionId).longValue() == -1)
				throw new RemoteException(
						"ProcessManagerError: There's no production. Make sure you have chosen a version of the process definition as production at least once.");

			return productionVersionId.toString();
		} catch (ObjectNotFoundException onfe) {
			throw new RemoteException("ProcessManagerError: No such processdefinition with id '" + pdid
					+ "'. Please check '" + pdid + "' is a definition id not a definition version id.", onfe);
		} catch (Exception e) {
			// e.printStackTrace();
			throw new RemoteException("ProcessManagerError:" + e.getMessage(), e);
		}
	...
	public String getProcessDefinitionProductionVersionByName(String pdName) throws RemoteException {
		...
		try {
			...
			if (pdlr == null)
				throw new RemoteException(
						"ProcessManagerError: There's no such process definition named '" + pdName + "'");
	...
	public String getProcessDefinitionProductionVersionByAlias(String alias) throws RemoteException {
		...
		try {
			...
			if (pdlr == null)
				throw new RemoteException(
						"ProcessManagerError: There's no such process definition aliased '" + alias + "'");
	...
	public ProcessInstanceRemote[] listProcessInstanceRemotes(String definition, String status) throws RemoteException {
		...
		try {
			...
			if (definition != null) {
				...
				/*
				 * else instances = pihr.findByStatus(status);
				 */
			}
	...
	/*
	 * public String[] listProcessInstanceIds() throws RemoteException{ try{
	 * return ActivityInstance.getInstanceIds(); }catch(Exception e){
	 * e.printStackTrace(); throw new
	 * RemoteException("ProcessManagerError:"+e.getMessage(), e); } }
	 * 
	 * public String[] listProcessInstanceIds(String definitionName) throws
	 * RemoteException{ try{ return
	 * ActivityInstance.getInstanceIds(definitionName); }catch(Exception e){
	 * e.printStackTrace(); throw new
	 * RemoteException("ProcessManagerError:"+e.getMessage(), e); } }
	 */

	public String addProcessDefinition(String name, int version, String description, boolean isAdhoc, String strDef,
			String folder, String belongingPdid, String alias) throws RemoteException {
		log("addProcessDefinition", new Object[] { name, new Integer(version), description, new Boolean(isAdhoc),
				strDef, folder, belongingPdid });
		try {
			HashMap options = new HashMap();
			if (alias != null)
				options.put("alias", alias);

			String[] defVerIdAndDefId = ProcessDefinitionFactory.getInstance(getTransactionContext()).addDefinitionImpl(
					belongingPdid, null, version, name, description, isAdhoc, strDef, folder, false, options);
	...
	public String addProcessDefinition(String name, int version, String description, boolean isAdhoc, String strDef,
			String folder, String belongingPdid, String alias, String objectType) throws RemoteException {
		log("addProcessDefinition", new Object[] { name, new Integer(version), description, new Boolean(isAdhoc),
				strDef, folder, belongingPdid });
		try {
			HashMap options = new HashMap();
	...
	public String addProcessDefinitionWithProgramId(String name, int version, String description, boolean isAdhoc, String strDef,
			String folder, String belongingPdid, String alias, String objectType, String programId, String author, String duration) throws RemoteException {
		log("addProcessDefinition", new Object[] { name, new Integer(version), description, new Boolean(isAdhoc),
				strDef, folder, belongingPdid });
		try {
			
			System.out.println("addProcessDefinitionWithProgramId::programId:"+programId);
			HashMap options = new HashMap();
			if (objectType != null)
				options.put("objectType", objectType);
			if (alias != null)
				options.put("alias", alias);
			if ( UEngineUtil.isNotEmpty(programId) ) 
				options.put("programId", programId);
			if ( UEngineUtil.isNotEmpty(author) ) 
				options.put("author", author);
			if ( UEngineUtil.isNotEmpty(duration) ) 
	...
	public String addProcessDefinition(String name, int version, String description, boolean isAdhoc, String strDef,
			String folder, String belongingPdid, String objectType, String alias, String superDefId)
			throws RemoteException {
		log("addProcessDefinition", new Object[] { name, new Integer(version), description, new Boolean(isAdhoc),
				strDef, folder, belongingPdid });
		try {
	...
	public String addProcessDefinition(String name, int version, String description, boolean isAdhoc,
			ProcessDefinition processDefinition, String folder, String belongingPdid) throws RemoteException {
		log("addProcessDefinition", new Object[] { name, new Integer(version), description, new Boolean(isAdhoc),
				processDefinition, folder, belongingPdid });
		try {
			String[] defVerIdAndDefId = ProcessDefinitionFactory.getInstance(getTransactionContext()).addDefinitionImpl(
					belongingPdid, null, version, name, description, isAdhoc, processDefinition, folder, false, null);
			return defVerIdAndDefId[1] + "@" + defVerIdAndDefId[0];
		} catch (Exception e) {
			e.printStackTrace();
			throw new RemoteException("ProcessManagerError:" + e.getMessage(), e);
	...
	public String addFolder(String folderName, String parentFolder) throws RemoteException {
		...
		try {
			...
			// if there is no parent folder, this will occur an exception to
			// break this try~catch block
	...
	public Serializable getActivityProperty(String processDefinition, String tracingTag, String propertyName)
			throws RemoteException {
		log("getActivityProperty", new Object[] { processDefinition, tracingTag, propertyName });
		try {
			if (!UEngineUtil.isNotEmpty(processDefinition))
				throw new UEngineException("Check the definition id");
			...
			Serializable activityProperty = (Serializable) activity.getClass()
					.getMethod("get" + propertyName, new Class[] {}).invoke(activity, new Object[] {});
	...
	public Serializable getActivityPropertyFromInstance(String instanceId, String tracingTag, String propertyName)
			throws RemoteException {
		log("getActivityProperty", new Object[] { instanceId, tracingTag, propertyName });
		try {
			if (!UEngineUtil.isNotEmpty(instanceId))
				throw new UEngineException("Check the instance id");
			...
			if (trcTags.length > 1) {
				...
				for (int i = 0, n = trcTags.length - 1; i < n; i++) {
					if (!(definition.getActivity(trcTags[i]) instanceof SubProcessActivity))
						break;
			...
			if (!(activity instanceof SubProcessActivity)) {
				propertyName = propertyName.substring(0, 1).toUpperCase() + propertyName.substring(1);// .toLowerCase();
				activityProperty = (Serializable) activity.getClass().getMethod("get" + propertyName, new Class[] {})
	...
	public String viewProcessDefinitionFlowChart(String processDefinition, Map options) throws RemoteException {
		...
		try {
			...
			// return
			// ProcessDefinitionViewer.getInstance().render(getDefinition(processDefinition),
			// null, options).toString();
		} catch (Exception e) {
			e.printStackTrace();
			throw new RemoteException("ProcessManagerError:" + e.getMessage(), e);
		}
	...
	public String viewProcessInstanceFlowChart(String instanceId, Map options) throws RemoteException {
		...
		try {
			if (instanceId == null)
				return "";
			// throw new RemoteException("ProcessManagerError: null process
			// instance id");

			ProcessInstance instance = getInstance(instanceId);
			ProcessDefinition definition = instance.getProcessDefinition();

			// for performance and synchronized view - we don't need to create
			// snapshot by the implementation of caching logic.
			...
			// return ProcessDefinitionViewer.getInstance().render(definition,
			// shotCopy, options).toString();
		} catch (Exception e) {
			e.printStackTrace();
			throw new RemoteException("ProcessManagerError:" + e.getMessage(), e);
		}
	...
	public Serializable sendMessage(String instanceId, String message, Serializable payload) throws RemoteException {
		...
		try {
			ProcessInstance instance = getInstance(instanceId);
			ProcessDefinition definition = instance.getProcessDefinition();
			return (Serializable) definition.fireMessage(message, instance, payload); // send
																						// message
																						// to
																						// the
																						// whole
	...
	public Serializable sendMessageXML(String instanceId, String message, String payload) throws RemoteException {
		...
		try {
			...
			return (Serializable) definition.fireMessageXML(message, instance, payload); // send
																							// message
																							// to
																							// the
																							// whole
	...
	public void completeWorkitem(String instanceId, String tracingTag, String taskId, ResultPayload payload)
			throws RemoteException {
		completeWorkitemImpl(instanceId, tracingTag, taskId, payload, false);
	}

	public void saveWorkitem(String instanceId, String tracingTag, String taskId, ResultPayload payload)
			throws RemoteException {
		completeWorkitemImpl(instanceId, tracingTag, taskId, payload, true);
	}

	private void completeWorkitemImpl(/* String definitionId, */String instanceId, String tracingTag, String taskId,
			ResultPayload payload, boolean saveOnly) throws RemoteException {

		logInst("completeWorkitem", new Object[] { instanceId, tracingTag, taskId, payload });
		try {

			/*
			 * boolean isInitiatingWorkItemCompletion =
			 * (!UEngineUtil.isNotEmpty(instanceId) &&
			 * UEngineUtil.isNotEmpty(definitionId));
			 * if(isInitiatingWorkItemCompletion){ instanceId =
			 * initializeProcess(definitionId);
			...
			if (!instance.isRunning("") && ((instance.isNew() && tracingTag.indexOf("@") > 0) // first
																								// process
																								// is
																								// subprocess
					|| !((HumanActivity) instance.getProcessDefinition().getActivity(tracingTag))
							.isNotificationWorkitem()))
				instance.execute();
			 ...
			 * String message = humanActivity.getMessage();
			 * 
			 * //add task id into payload KeyedParameter[] extendedValues =
			 * payload.getExtendedValues(); int evSize=0;
			 * if(extendedValues!=null) evSize = extendedValues.length;
			 * KeyedParameter[] extendedValues_ = new KeyedParameter[evSize+1];
			 * System.arraycopy(extendedValues, 0, extendedValues_, 0,
			 * extendedValues.length); extendedValues_[evSize] = new
			 * KeyedParameter(HumanActivity.PAYLOADKEY_TASKID, taskId);
	...
	private String getProcessDefinition(String processDefinition, String encodingStyle, boolean withoutInheritance)
			throws RemoteException {
	...
	public String getResource(String resourceId) throws RemoteException {
		...
		try {
			InputStream ris = ProcessDefinitionFactory.getInstance(getTransactionContext())
	...
	/**
	 * This method is costly to call. Use getProcessDefinitionRemote() instead
	 * if full data is not neccessary.
	 */
	...
	private String getProcessDefinitionWithInstanceId(String instanceId, String encodingStyle,
			boolean withoutInheritance) throws RemoteException {
	...
	public ProcessDefinitionRemote getProcessDefinitionRemote(String pdvid) throws RemoteException {
		try {
			ProcessDefinitionVersionRepositoryHomeLocal pdvrhl = GlobalContext
					.createProcessDefinitionVersionRepositoryHomeLocal(getTransactionContext());
	...
	public ProcessDefinitionRemote getProcessDefinitionRemoteWithInstanceId(String instanceId) throws RemoteException {
		try {
			...
			if (definition == null)
				throw new UEngineException(
						"Can't find definition for this instance. Check if the definition file exists.");
	...
	public ProcessDefinition getProcessDefinitionWithInstanceId(String instanceId) throws RemoteException {
		...
		try {
			// ProcessInstanceRepositoryLocal pil =
			// GlobalContext.createProcessInstanceRepositoryHomeLocal().findByPrimaryKey(new
			// Long(instanceId));

			ProcessInstance instance = getInstance(instanceId);
			ProcessDefinition orginial = instance.getProcessDefinition();

			// ProcessDefinition orginial =
			// getProcessDefinition(""+pil.getDefVerId());

			// Some EJB containers doesn't carry out serialization for object
			// passing (especially in case that the caller is in the same VM).
			// So, it is required to make a copy so that the original object
			// cannot be modified.
	...
	public ProcessInstance getProcessInstance(String instanceId, String executionScope) throws RemoteException {
		...
		try {
			...
			// The request for reseting the executionscope with null value
			// should be ignored
			// because it's intent is the requestor doesn't know the
			// executionScope,
			// so existing execution scope value stored in the cached instance
			// should be kept.
			// This logic is related to event scope mechanism malfunction,
			// specifically to the evenhandler activity's instance data
			// allocation.
			if (executionScope != null)
				instance.setExecutionScope(executionScope);

			// it is ok to return the original object since we have implemented
			// caching logic.
	...
	public Serializable getActivityInstanceProperty(String instanceId, String tracingTag, String propertyName)
			throws RemoteException {
		logInst("getActivityInstanceProperty", new Object[] { instanceId, tracingTag, propertyName });
		try {
			if (instanceId == null)
				return null; // guard against illegal requests
			...
			Object returnVal;
			try {
				returnVal = activity.getClass().getMethod("get" + propertyName, new Class[] { ProcessInstance.class })
						.invoke(activity, new Object[] { instance });
			} catch (NoSuchMethodException e) {
				try {
					returnVal = activity.getClass()
							.getMethod("is" + propertyName, new Class[] { ProcessInstance.class })
							.invoke(activity, new Object[] { instance });
	...
	public void setActivityInstanceProperty(String instanceId, String tracingTag, String propertyName,
			Serializable value) throws RemoteException {
		// logInst("setActivityInstanceProperty", new Object[]{instanceId,
		// tracingTag, propertyName, value});
		setActivityInstanceProperty(instanceId, tracingTag, propertyName, value, null);
	}

	public void setActivityInstanceProperty(String instanceId, String tracingTag, String propertyName,
			Serializable value, Class valueType) throws RemoteException {
		logInst("setActivityInstanceProperty", new Object[] { instanceId, tracingTag, propertyName, value, valueType });
		try {
			if (instanceId == null)
				return; // guard against illegal requests
			if (valueType == null) {
				if (value == null)
					throw new UEngineException(
							"Property value should be not null. If you want to apply with null value, provide valueType as well."); 
			...
			if (activity == null)
				throw new UEngineException("No such activity with tracing tag : " + tracingTag + " in definition '"
						+ definition.getId() + "'");

			propertyName = propertyName.substring(0, 1).toUpperCase() + propertyName.substring(1);// .toLowerCase();
			// the pattern of instance property (stateful) setting method is
			// 'setXXX(instance, value)'
			activity.getClass().getMethod("set" + propertyName, new Class[] { ProcessInstance.class, valueType })
					.invoke(activity, new Object[] { instance, value });
		} catch (Exception e) {
			e.printStackTrace();
			throw new RemoteException("ProcessManagerError:" + e.getMessage(), e);
		}
	...
	public Serializable doActivityAction(String instanceId, String tracingTag, String actionName,
			Serializable[] parameters, Class[] parameterTypes) throws RemoteException {
		...
		try {
			...
			return (Serializable) activity.getClass().getMethod(actionName, actualParameterTypes).invoke(activity,
					actualParameters);
	...
	public void removeProcessDefinition(String processDefinition) throws RemoteException {
		...
		try {
			...
			ProcessInstanceRepositoryHomeLocal pihr = GlobalContext.createProcessInstanceRepositoryHomeLocal();
			// if there is any of instance of this folder, this definition can't
			// be removed
	...
	private ProcessDefinition getDefinition(String pdvid, boolean withoutInheritance) throws Exception {
		...
		ProcessDefinition pd = ProcessDefinitionFactory.getInstance(getTransactionContext()).getDefinition(pdvid, true,
				withoutInheritance);

		/*
		 * if(pd.isAdhoc()) pd = ProcessDefinitionFactory.getDefinition(name,
		 * false);
		 */
	...
	private void changeProcessDefinitionImpl(String instanceId, Object definition) throws RemoteException {
		try {
			...
			if (processDefinition == instance.getProcessDefinition())
				throw new UEngineException(
						"Dynamic change exception: Changed definition is the original one so it can't be changed. That means your code didn't clone the definition for changing it. Also It implies the damage of cached definition.");
	...
	// private void changeProcessDefinitionImpl(String instanceId, Object
	// definition) throws RemoteException{
	// try{
	// //verify whether the instance flow has been changed and the changed
	// definition is inconsistence with instance changes.
	//
	//
	// isAdhocInstance
	//// ProcessInstanceRepositoryLocal pil =
	...
	//
	// ProcessDefinitionVersionRepositoryLocal pdvl =
	// GlobalContext.createProcessDefinitionVersionRepositoryHomeLocal(getTransactionContext()).findByPrimaryKey(new
	// Long(orginalDefinition.getId()));
	//
	...
	// options.put("associatedInstance", instance); // lets
	// "addDefinitionImpl()" change this instance's definition as well
	//
	// ProcessDefinitionFactory definitionFactory =
	// ProcessDefinitionFactory.getInstance(getTransactionContext());
	//
	// String defVerId[] =
	// definitionFactory.addDefinitionImpl(""+pdvl.getDefId(), pvId, -1, null,
	// null, true, definition, null, bOverwrite, options);
	// //if new one, modify the old link to indicate new one.
	// if(!bOverwrite){
	/// * ProcessInstanceRepositoryHomeLocal pihr =
	// GlobalContext.createProcessInstanceRepositoryHomeLocal();
	// ProcessInstanceRepositoryLocal piRemote = pihr.findByPrimaryKey(new
	// Long(instanceId));
	// */
	...
	// //instance = getInstance(instanceId);//issue new instance so that the
	...
	public ActivityReference getInitiatorHumanActivityReference(String pdvid) throws RemoteException {
		try {
			...
			return pd.getInitiatorHumanActivityReference(null, getTransactionContext());
		} catch (Exception e) {
			throw new RemoteException("ProcessManagerError:" + e.getMessage(), e);
		}
	}
	...
	public String initialize(String processDefinition, String instanceId, RoleMapping loggedRoleMapping)
			throws RemoteException {
	...
	public void exportProcessDefinitionbyVersionId(String defVerId) throws RemoteException {
		...
		try {
			...
			for (int i = 0; i < subDefinition.size(); i++) {
				...
				ZipEntryMapper entryMapper = new ZipEntryMapper(defName, subDefVerId, pdr.getAlias(), objType,
						pdr.getVersion(), in);
				zipEntryMapperList.add(entryMapper);

				ua.setDefinitionList(defName, pdr.getAlias(), pdr.getBelongingDefinitionId(), subDefVerId, objType, "",
						"", pdr.getParentFolder(), pdr.getId(), isRoot);
			}
	...
	static {
		TEMP_DIRECTORY = GlobalContext.getPropertyString("server.definition.path",
	...
	public void exportProcessDefinitionbyDefinitionId(String defId, boolean ExportAllVersion) throws Exception {
		try {
			...
			ua.setDefinitionList(defName, "", pdr.getBelongingDefinitionId(), String.valueOf(pdr.getVersion()),
					ZipEntryMapper.TYPE_FOLDER, defName, "", "-1", pdr.getId(), true);
			...
			setDefinitionsForExport(defId, listProcessDefinitionRemotesLight(), defName + File.separatorChar, options,
					ExportAllVersion);
	...
	private Hashtable setFormsForExport(String rootDirectory, Hashtable options) throws Exception {
		...
		for (int i = 0; i < ua.getDefinitionList().size(); i++) {
			...
			if (!"process".equals(da.getObjectType()))
				continue;
					...
					if (dv instanceof org.uengine.contexts.HtmlFormContext) {
						...
						if (UEngineUtil.isNotEmpty(formDefId)) {
							...
							if (!ua.containKeys(formPdr.getBelongingDefinitionId())) {
								...
								ua.setDefinitionList("_Form", "", "_Form", "", ZipEntryMapper.TYPE_FOLDER, "_Form", "",
										ua.getMainProcessDefinition().getBelongingId(), "", false);

								String fileName = ZipEntryMapper.ENTRY_SEPARATOR + formPdr.getName().getText()
										+ ZipEntryMapper.ENTRY_SEPARATOR + formPdr.getAlias()
										+ ZipEntryMapper.ENTRY_SEPARATOR + formPdr.getId()
										+ ZipEntryMapper.ENTRY_SEPARATOR + formPdr.getObjType();
								String desc = formPdr.getDescription() == null ? ""
										: formPdr.getDescription().getText();

								ua.setDefinitionList(formPdr.getName().getText(), formPdr.getAlias(),
										formPdr.getBelongingDefinitionId(), String.valueOf(formPdr.getVersion()),
										formPdr.getObjType(), fileName, desc, "_Form", formPdr.getId(), false);
								getFilePathNFileCopy(formPdr,
										rootDirectory + File.separatorChar + "_Form" + File.separatorChar, false);
							}
						}
					}
				}
	...
	private Hashtable setDefinitionsForExport(String processDefinition, ProcessDefinitionRemote[] pdrs,
			String parentDirectory, Hashtable options, boolean ExportAllVersion) throws Exception {
		for (int i = 0; i < pdrs.length; i++) {
			ProcessDefinitionRemote pdr = pdrs[i];

			if (!pdr.getParentFolder().equals(processDefinition))
				continue;

			UEngineArchive ua = (UEngineArchive) options.get(UEngineArchive.UENGINE_ARCHIVE);

			if (pdr.isFolder()) { // folder
				ua.setDefinitionList(pdr.getName().getText(), "", pdr.getBelongingDefinitionId(),
						String.valueOf(pdr.getVersion()), ZipEntryMapper.TYPE_FOLDER, pdr.getName().getText(), "",
						pdr.getParentFolder(), pdr.getId(), false);
				options.put(UEngineArchive.UENGINE_ARCHIVE, (UEngineArchive) ua);
				...
				ua.setDefinitionList(pdr.getName().getText(), pdr.getAlias(), pdr.getBelongingDefinitionId(),
						String.valueOf(pdr.getVersion()), pdr.getObjType(),
						pdr.getName().getText() + ZipEntryMapper.ENTRY_SEPARATOR + pdr.getAlias()
								+ ZipEntryMapper.ENTRY_SEPARATOR + pdr.getId() + ZipEntryMapper.ENTRY_SEPARATOR
								+ pdr.getObjType(),
						pdr.getDescription() == null ? "" : pdr.getDescription().getText(), pdr.getParentFolder(),
						pdr.getId(), false);
				options.put(UEngineArchive.UENGINE_ARCHIVE, (UEngineArchive) ua);
				...
				if (pdr.getObjType().equals("process")) {
					...
					try {
						...
						_DUMMY_LIST_.add(pdr.getId() + "(" + pdr.getName() + "," + pdr.getObjType() + ","
								+ pdr.getAlias() + ") ");
					}
				}
			}
	...
	private Hashtable setSubProcessesForExport(String rootDirectory, Hashtable options) throws Exception {
		...
		while (keys.hasMoreElements()) {
			...
			if (!ua.containKeys(pdr.getBelongingDefinitionId())) {
				// _SubProc folder create
				ua.setDefinitionList("_SubProc", "", "_SubProc", "", ZipEntryMapper.TYPE_FOLDER, "_SubProc", "",
						ua.getMainProcessDefinition().getBelongingId(), "", false);

				// add sub-process
				String fileName = ZipEntryMapper.ENTRY_SEPARATOR + pdr.getName().getText()
				...
				ua.setDefinitionList(pdr.getName().getText(), pdr.getAlias(), pdr.getBelongingDefinitionId(),
						String.valueOf(pdr.getVersion()), pdr.getObjType(), fileName, desc, "_SubProc", pdr.getId(),
						false);
	...
	private void getFilePathNFileCopy(ProcessDefinitionRemote pdr, String parentDirectory, boolean isSub)
			throws Exception {
		...
		if (def.startsWith("LINK:")) {
			...
			if (isSub == false) {
				filePath = TEMP_DIRECTORY + parentDirectory
						+ (pdr.getName().getText() + ZipEntryMapper.ENTRY_SEPARATOR + pdr.getAlias()
								+ ZipEntryMapper.ENTRY_SEPARATOR + pdr.getId() + ZipEntryMapper.ENTRY_SEPARATOR
								+ pdr.getObjType());
			} else if (isSub == true) {
				filePath = TEMP_DIRECTORY + parentDirectory
						+ ("sub" + ZipEntryMapper.ENTRY_SEPARATOR + pdr.getName().getText()
								+ ZipEntryMapper.ENTRY_SEPARATOR + pdr.getAlias() + ZipEntryMapper.ENTRY_SEPARATOR
								+ pdr.getId() + ZipEntryMapper.ENTRY_SEPARATOR + pdr.getObjType());
			}
	...
	public Vector importProcessDefinition(String parentFolder, InputStream loadedZipFile, UEngineArchive editedUa,
		...
		try {
			...
			UEngineArchive ua = (UEngineArchive) GlobalContext
					.deserialize((String) inputStreamList.get("manifest.xml"));
			...
			while (keys.hasMoreElements()) {
				...
				String toFindDefid = (String) newDefIdList.get(key); // New
																		// DefId
	...
	private void importFolder(String newRootFolderId, String oldRootFolderId, UEngineArchive ua,
			UEngineArchive edited_ua, Hashtable newDefIdList) throws Exception {

		for (int i = 0; i < ua.getDefinitionList().size(); i++) {
			DefinitionArchive da = (DefinitionArchive) ua.getDefinitionList().get(i);
			DefinitionArchive fixedDa = (DefinitionArchive) edited_ua.getDefinitionList().get(i);

			if (da.isRoot() == false && da.getObjectType().equals("folder")
					&& da.getParentFolder().equals(oldRootFolderId)) {
	...
	private void importDefinitions(Hashtable inputStreamList, String[] command, UEngineArchive ua,
			UEngineArchive edited_ua, Hashtable newDefIdList, Hashtable newDefVerIdList, Hashtable newAliasList)
			throws Exception {
		...
		for (int i = 0; i < ua.getDefinitionList().size(); i++) {
			...
			if (!da.isRoot() && fixedDa.getArchiveFileName().equals(da.getArchiveFileName())
					&& !"folder".equals(da.getObjectType())) {
				...
				if (definitionDoc != null) {
					...
					if ("new".equals(command[i])) {
						...
						deployedDefinitionInformation = ProcessDefinitionFactory.getInstance(getTransactionContext())
								.addDefinitionImpl(null, null, 1, fixedDa.getName(), null, false, definitionDoc,
										parentFolder, false, objectOptions);
						...
						for (int j = 0; j < pdrs.length; j++) {
							...
							if ((!tempPdr.isFolder) && ((tempPdr.getAlias()).equals(fixedDa.getAlias()))
									&& (fixedDa.getAlias() != null && !fixedDa.getAlias().equals(""))) {
						...
						deployedDefinitionInformation = ProcessDefinitionFactory.getInstance(getTransactionContext())
								.addDefinitionImpl(pdId, null, versionId + 1, fixedDa.getName(), null, false,
										definitionDoc, parentFolder, false, objectOptions);
					}
	...
	private void relateDefinitionsForImport(UEngineArchive ua, UEngineArchive edited_ua, Hashtable newDefIdList,
			Hashtable newDefVerIdList, Hashtable newAliasList) throws Exception {
				...
				public void logic(Activity act) {
					try {
						if (act instanceof SubProcessActivity) {
							...
							String oldAlias = (spAct.getDefinitionId().split("@")[0]).substring(1,
									(spAct.getDefinitionId().split("@")[0]).length() - 1);
	...
	public HashMap getInitiationParameterMap(String pvdid) throws RemoteException {
		try {
			...
			if (initiatorHumanActivityReference != null) {
				...
				/*
				 * String tool = initiatorHumanActivity.getTool(); String url =
				 * "../wih/" + tool + "/index.jsp";
				 */
	...
	public String getProcessDefinitionWithInstanceIdWithoutInheritance(String instanceId, String encodingStyle)
			throws RemoteException {
		return getProcessDefinitionWithInstanceId(instanceId, encodingStyle, true);
	}

	public String getProcessDefinitionWithoutInheritance(String defVerId, String encodingStyle) throws RemoteException {
		return getProcessDefinition(defVerId, encodingStyle, true);
	}

	public String addFolder(String folderName, String parentFolder,
			String comCode) throws RemoteException {
		log("addFolder", new Object[] { folderName, parentFolder });
		try {
			ProcessDefinitionRepositoryHomeLocal pdhr = GlobalContext
					.createProcessDefinitionRepositoryHomeLocal(getTransactionContext());
