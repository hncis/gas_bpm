...
public class ProcessDefinitionRemote implements java.io.Serializable{
	...
	public ProcessDefinitionRemote(ProcessDefinition pd, ProcessTransactionContext ptc) throws Exception{
		this(pd, ptc, null);
	}

	public ProcessDefinitionRemote(ProcessDefinition pd, ProcessTransactionContext ptc, ProcessDefinitionVersionRepositoryLocal pdvrl) throws Exception{
		roles = pd.getRoles();
		name = pd.getName();		
		...
		if(ptc!=null && isInitiateByFirstWorkitem()){
			HumanActivity initiatorHumanActivity = (HumanActivity) pd.getInitiatorHumanActivityReference(null, ptc).getActivity();
