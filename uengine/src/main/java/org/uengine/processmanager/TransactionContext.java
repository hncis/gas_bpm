...
public class TransactionContext implements ConnectionFactory{
	...
	// 2018-06-12
	private List<Object> lobList = new ArrayList<Object>(); //append
	public List<Object> getLobList() { // append
		return lobList;
	}
	public void setLobList(List<Object> lobList) { //append
		this.lobList = lobList;
	}
	
	private void releaseLobData(){ // append
		for(Object lob : lobList){
			if(lob != null && lob instanceof Clob){
				try{((Clob)lob).free();}catch(SQLException e){e.printStackTrace();}
			}
			if(lob != null && lob instanceof Blob){
				try{((Blob)lob).free();}catch(SQLException e){e.printStackTrace();}
			}
		}
