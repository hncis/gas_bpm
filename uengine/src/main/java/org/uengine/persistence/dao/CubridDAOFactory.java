				...
				public int call() throws Exception {
						...
						if (var_taskId == null) {
							IDAO valueDAO = ConnectiveDAO.createDAOImpl(
										getConnectionFactory(),
										"select FOR SEQ_BPM_WORKITEM.NEXT_VALUE as TASKID from db_root",
										IDAO.class
							);
							valueDAO.select();
							valueDAO.next();
...
//	select bpm_func_reserveWorkItem(100, 'title','de', 'e','s',1,null,null,null,'','','','') from db_root;
/*	public static Long bpm_func_reserveWorkItem(
		...
		if(var_taskId==null){
			IDAO valueDAO = GenericDAO.createDAOImpl(
				...
				"select NEXT VALUE FOR SEQ_BPM_WORKITEM as TASKID from db_root",
				IDAO.class
			);
			valueDAO.select();
			valueDAO.next();
...
*/
	public KeyGeneratorDAO createKeyGenerator(String forWhat, Map options) throws Exception {
		return (KeyGeneratorDAO) create(
				...
				"select SEQ_BPM_" + forWhat + ".NEXT_VALUE as keyNumber from db_root");
	}

	public Calendar getNow() throws Exception {
		String sql = "select CURRENT_TIMESTAMP as NOW from db_root";
